...
Unit::Unit(bool sd, int row, int col, GoblinType type, Direction dir, int ap)
    : side(sd), row(row), col(col), gType(type), gDir(dir), ap(ap)
{

}

// Constructor for tracking goblins
Unit::Unit(bool sd, int row, int col, GoblinType type, int fov)
    : side(sd), row(row), col(col), gType(type), fov(fov)
{
